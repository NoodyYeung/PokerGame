...
//					System.out.println("[Debug] identifyPattern:" + lastPattern.getClass().getName());
	...
	public Pattern identifyPattern(List<Card> cards) {
		...
		if(cards.size()==1) { // TODO : condition 1
			pattern = new Pone(cards.get(0).getNumber()); 
			return pattern;
		}
				...
				if(cards.size()<5) // TODO : condition 2
					...
					if(isBomb(cards)) { // TODO : condition 4a (line 72 + 73)
						pattern = new Pbomb(cards.get(0).getNumber());
						return pattern;
					}
					else {
						if(cards.size()==3) { // TODO : condition 4b (line 72 + 78)
							pattern = new Pthree(cards.get(0).getNumber());
						}
						else {
							pattern = new Ptwo(cards.get(0).getNumber());
						}
						return pattern;
					}
	...
	public boolean isRocket(List<Card> cards) {
		return cards.size() == 2 && (
				(cards.get(0).getNumber()==53 && cards.get(1).getNumber()==54) || // TODO 5a : decision coverage
				(cards.get(1).getNumber()==53 && cards.get(0).getNumber()==54));
	}
